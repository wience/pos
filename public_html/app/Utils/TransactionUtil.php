    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            foreach ($details['lines'] as $line) {
                ...
                $total_line_discount += $line['line_discount_uf'];
                $total_line_taxes += $line['tax_unformatted'];
            }
    ...
    */
    ...
    public function getLedgerDetails($contact_id, $start, $end)
    {
        ...
        $ledger = array_merge([[
            ...
            'total' => '',
            'payment_method' => '',
            'debit' => $contact->type == 'supplier' ? abs($total_opening_bal) : '',
            'credit' => $contact->type == 'customer' ? abs($total_opening_bal) : '',
            'others' => ''
        ]], $ledger) ;
